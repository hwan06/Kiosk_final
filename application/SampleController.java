...
public class SampleController {
    ...
    public void OrderButtonAction(ActionEvent event) {
    	//총주문금액이 0이면 
    	if(sumLabel.getText() == "0") {
    		Alert alert = new Alert(AlertType.INFORMATION); // 메세지 출력(메뉴선택, 계산하기 버튼 클릭후 주문해)
    		alert.setContentText("계산하기 버튼 클리후 주문해");
    		alert.show();
    	}else { //그 외에는(총주문금액이 0이 아니면)
    		DBconnect DBconn = new DBconnect();
    		Connection conn = DBconn.getConnection(); // DB접속
    		
    		String sql = "insert into orderlist_accounts"
  				  + " (idx, order_time, menu1, count1,"
  				  + " menu2, count2, menu3, count3, total)"
    		...
    		try {
				PreparedStatement ps = conn.prepareStatement(sql);// 쿼리 실행
				...
				if(rs.next()) {
					...
					Alert alert = new Alert(AlertType.INFORMATION);
					alert.setContentText("주문오류:카운터에 문의");
				}
